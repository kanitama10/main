...
var Feed = (function() {
    ...
    Feed.prototype.load = function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $rss.find('item').each(function() {
                    var item = this;
                   //html_liに表示するニュースのhtml形式を保存する
                    html_li.push(self.createListElement(item));
                    count ++;
                });
            },
            error: function() {
        ...
        var timer = setInterval(function(){
            if(sec == 0){
                $(self.newsEl).fadeIn(1000);
                $(self.newsEl).html(html_li[i]);
                sec++;
            }else if(sec == 5){
                $(self.newsEl).fadeOut(1000);
                sec = 0;
            }else{
                sec++;
            }
    ...
    Feed.prototype.addClickHandler = function() {
        $(this.newsEl).on('click', 'div', function() {
    ...
    Feed.prototype.createListElement = function(item) {
        ...
        return '<div class="feed-item" data-link="' + link + '">' + 
